...
# list = html.xpath('//div[@class="vervideo-bd"]')
# video_urls = []
# countid_list = []
# video_names = []
# download_urls = []
...
# for li in list:
#     title = li.xpath('./a/div[@class="vervideo-title"]/text()')
#     video_names.append(title)
#     incomplete_link = li.xpath("./a/@href")[0]
#     detailed_link = "https://www.pearvideo.com/" + li.xpath("./a/@href")[0]
#     #print(title,":",detailed_link)
#     detailed_page_text = requests.get(url=detailed_link,headers=headers).text
#     #在视频详细页面的element里面没有video的url，说明是动态加载？要到network里面去找
#     match = re.search(r"\d+",incomplete_link)
#     countid = match.group()
#     countid_list.append(countid)
#     #print(countid)
#     mrd = random.random()
#     video_url=f"https://www.pearvideo.com/videoStatus.jsp?contId={countid}&mrd={mrd}"
#     #print(video_url)
...
#     response = requests.get(url,headers=headers_video).json
#     time.sleep(1)
#     download_url = response['videoInfo']['videos']['scrUrl']
#     download_url = re.sub('/[gh]')
